...
class Draw_MPC_tracking:
    def __init__(self, rob_diam, init_state, robot_states):
    ...
    def draw_trajectory(self):
        fig, ax = plt.subplots()
        # Plotting the initial position as a circle
        circle = plt.Circle((self.init_state[0], self.init_state[1]), self.rob_diam/2, color='b', fill=False)
        ax.add_artist(circle)
        trajectory = plt.Circle((0, 0), 1, color='k', fill=False)
        ax.add_artist(trajectory)
        obstacle = plt.Circle((1, 0), 0.3, color='g', fill=False)
        ax.add_artist(obstacle)
        # Plotting the trajectory
        x_vals = [state[0] for state in self.robot_states]
        y_vals = [state[1] for state in self.robot_states]
        ax.plot(x_vals, y_vals, 'r', label="Trajectory")
